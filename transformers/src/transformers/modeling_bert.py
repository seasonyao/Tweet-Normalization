...
class myBertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # for every token, it has 2 types alignment, keep(0) or norm(1)
        self.token_align_embeddings = nn.Embedding(2, config.hidden_size)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, token_align_ids=None):
        ...
        if token_align_ids is None:
            token_align_ids = torch.zeros(input_shape, dtype=torch.long, device=self.position_ids.device)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, token_align_ids=token_align_ids
        )
    ...
    def forward(
        ...
        **kwargs
        ...
        outputs = self.bert(
            ...
            token_align_ids=token_align_ids,
...
class BertForMultipleChoice(BertPreTrainedModel):
    ...
    def forward(
        ...
        token_align_ids = token_align_ids.view(-1, token_align_ids.size(-1)) if token_align_ids is not None else None
