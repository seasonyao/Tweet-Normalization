...
class Trainer:
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        ...
        signature_columns += ["is_labels_need_change"]
...
#                 )
    def setup_wandb(self):
        ...
        if self.is_world_process_zero():
            ...
            ENTITY, PROJECT, NAME = self.args.run_name.split()
            wandb.init(
                entity=ENTITY,project=os.getenv("WANDB_PROJECT", PROJECT), config=combined_dict, name=NAME)
#             wandb.init(
#                 entity="zonghaiyao",project=os.getenv("WANDB_PROJECT", "tweetnorm"), config=combined_dict, name=self.args.run_name
#             )
            # keep track of model topology and gradients, unsupported on TPU
            if not is_torch_tpu_available() and os.getenv("WANDB_WATCH") != "false":
                wandb.watch(
    ...
    def _prediction_loop(
        ...
        is_labels_need_change: list = None
        model.eval()
        if is_torch_tpu_available():
            ...
            if not prediction_loss_only:
                ...
                if inputs.get("input_ids") is not None:
                    if input_ids is None:
                        ...
                        input_ids = torch.cat((input_ids, inputs["input_ids"].detach()), dim=0)
                if inputs.get("is_labels_need_change") is not None:
                    if is_labels_need_change is None:
                        is_labels_need_change = inputs["is_labels_need_change"]
                    else:
                        is_labels_need_change.extend(inputs["is_labels_need_change"])
        if self.args.local_rank != -1:
            ...
            if input_ids is not None:
                input_ids = self.distributed_concat(input_ids, num_total_examples=self.num_examples(dataloader))
        elif is_torch_tpu_available():
            ...
            if input_ids is not None:
                input_ids = xm.mesh_reduce("eval_input_ids", input_ids, torch.cat)
        ...
        if self.compute_metrics is not None and preds is not None and label_ids is not None and input_ids is not None and is_labels_need_change is not None:
            if self.my_trainer:
                metrics = self.compute_metrics(my_EvalPrediction(predictions=preds, label_ids=label_ids, is_labels_need_change=is_labels_need_change))
            else:
    ...
    def prediction_loop(
        ...
        if self.args.local_rank != -1:
            ...
            if input_ids is not None:
                input_ids = distributed_concat(input_ids, num_total_examples=self.num_examples(dataloader))
        elif is_torch_tpu_available():
            # tpu-comment: Get all predictions and labels from all worker shards of eval dataset
            if preds is not None:
        ...
        if input_ids is not None:
            input_ids = nested_numpify(input_ids)
