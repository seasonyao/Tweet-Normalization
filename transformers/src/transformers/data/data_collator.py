...
def default_data_collator(features: List[InputDataClass]) -> Dict[str, torch.Tensor]:
    ...
    first = features[0]
    batch = {}
    batch['is_labels_need_change'] = [example["is_labels_need_change"] for example in features]
    ...
    for k, v in first.items():
        if k not in ("label", "label_ids", "is_labels_need_change") and v is not None and not isinstance(v, str):
